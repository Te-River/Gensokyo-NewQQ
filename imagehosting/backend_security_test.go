...
func TestImageHostingHTTPClientHasTimeout(t *testing.T) {
	if providerHTTPClient.Timeout != 15*time.Second {
		t.Fatalf("HTTP client timeout = %v", providerHTTPClient.Timeout)
	}
...
func TestImageHostRedirectPolicy(t *testing.T) {
	...
	if err := providerHTTPClient.CheckRedirect(makeRequest("https://cdn.example.com/image.png"), nil); err != nil {
		t.Fatalf("safe redirect rejected: %v", err)
	}
	if err := providerHTTPClient.CheckRedirect(makeRequest("https://127.0.0.1/image.png"), nil); err == nil {
	...
	if err := providerHTTPClient.CheckRedirect(makeRequest("https://example.com/next"), via); err == nil {
