...
`[CQ:remove,user_id=<用户ID>]` 会撤回目标用户在该群的最后一条消息。可选传入 `message_id=<消息ID>` 撤回指定消息；`msg_id` 作为兼容别名保留。用户、群和消息 ID 默认按现有 idmap 规则自动解析。详见 [CQ remove](./cq码/扩展CQ码/扩展cq码-cq-remove.md)。
