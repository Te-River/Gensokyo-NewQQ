...
入站解析时，`ProcessCQActive` 从文本中移除该 CQ 码并在 `foundItems` 中记录。出站发送时，`send_group_msg` 检测到 `active` 标记后清空缓存的 `msg_id` 和 `event_id`，强制走主动推送通道。`send_private_msg_wakeup` 中也会跳过 `active` 相关 key 避免误发媒体。
