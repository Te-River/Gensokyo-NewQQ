...
| `[CQ:set_group]` | `q群 (Group Chat)` | 出站动作统一码：成员禁言/解禁（`action=ban`）、全员禁言开关（`action=whole_ban`）、入群申请审批（`action=add_request`）、审批策略执行/删除（`action=strategy_execute`/`strategy_delete`）。 | [查看](./扩展CQ码/扩展cq码-cq-set_group.md) |
