...
- `GROUP_MESSAGE_CREATE`（普通群消息事件）→ 检测 `Mentions` 中是否有 `IsYou=true`，有为 `true`，否则为 `false`
...
在 `GROUP_MESSAGE_CREATE`（普通群消息）中，若内容包含对其他用户/机器人的 `@`，Gensokyo 会自动将原始 `<@OpenID>` 替换为标准的 `[CQ:at,qq=虚拟ID]` 格式，并建立 OpenID 与虚拟 ID 的映射，确保同一用户多次出现时 @ 虚拟 ID 始终一致。
