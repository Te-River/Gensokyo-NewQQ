...
> **分支说明（2026-08-04）**：为降低编译产物体积（UPX 后 14.9MB → 12.3MB），`main` 已回退至 `59a8455`（依赖升级前）。本版本（Release012）的全部改动（Dependabot 配置、依赖升级、CVE 修复、CodeQL 统一等）已完整保留在 `Refactor` 分支，未随 `main` 回退丢失。
...
**问题：** 未开启全量群消息（仅订阅 `GROUP_AT_MESSAGE_CREATE` 被动消息）的群中，`add_at_group` 配置会无条件添加 `[CQ:at,qq=AppID]`，但被动消息本身已包含对 bot 的 @（`remove_at` 配置未剥离时），导致出站消息出现重复 @（如 `@用户 @用户 回复内容`）。

**修复：** `ProcessGroupMessage.go` 中 `add_at_group` 添加 `[CQ:at,qq=AppID]` 的条件增加 `GetRemoveAt()` 判断——仅当 `remove_at` 开启（即 @bot 已被剥离需要补回）时才添加，避免已包含 @bot 的消息再重复添加。
